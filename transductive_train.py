...
# GCN_NFM:  input dense feature
# GCN:      input dense feature
###############
...
flags.DEFINE_bool('residual', True, 'Whether to use residual connection.')
...
def main(unused_argv):
  ...
  (full_adj, train_feats, test_feats, y_train, y_val, y_test,
          train_mask, val_mask, test_mask, train_data, val_data, test_data,
          num_data) = utils.load_ne_data_transductive(FLAGS.data_prefix, FLAGS.dataset, list(map(float,FLAGS.split)))
  
  # Partition graph and do preprocessing
  if FLAGS.bsize > 1: # multi cluster per epoch
    # transductive all data can be seen
    ...
    (parts, features_batches, support_batches, y_train_batches,
     train_mask_batches) = utils.preprocess(full_adj, train_feats, y_train,
    ...
    if FLAGS.bsize > 1:
      (features_batches, support_batches, y_train_batches,
       train_mask_batches) = utils.preprocess_multicluster(
           full_adj, parts, train_feats, y_train, train_mask,
